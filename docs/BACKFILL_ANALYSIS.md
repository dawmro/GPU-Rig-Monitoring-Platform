...
### FK Constraints
- Parent snapshots inserted first
- Child rows reference new parent IDs via mapping
- Error occurrences reference existing `metrics_lasterrors` rows (no new errors created)

### NOT NULL Constraints
- All NOT NULL columns are populated from source data
- `snapshot_id` in child rows is always valid (mapped from parent)

### Unique Constraints
- All tables have unique constraints on (rig_uuid, timestamp, ...) combinations
- `INSERT ... ON CONFLICT DO NOTHING` handles overlapping data gracefully
- When source data is shifted into a range that already has data, duplicates are silently skipped
- This is critical because the error table (and potentially other tables) may have pre-existing data outside the source window

### ID Generation
- New IDs are auto-generated by PostgreSQL sequence
- Old IDs are never reused
- Mapping is per-repetition (old_id → new_id for that repetition only)
- If a snapshot already exists (ON CONFLICT), its old_id is NOT added to the mapping, so child rows referencing it are also skipped (correct behavior — they already exist)

## Overlapping Data Problem

**Discovery:** The error table had 18 hours of pre-existing data, not just 9. When
shifting source data back by 9 hours, the first repetition's target range
`[now-18h, now-9h]` overlapped with existing data `[now-18h, now]`, causing
unique constraint violations on `(rig_uuid, timestamp, error_event_id)`.

**Solution:** All INSERT statements use `ON CONFLICT DO NOTHING`. This means:
- Rows that already exist are skipped (no error)
- Only genuinely new rows are inserted
- The script is idempotent — safe to re-run after partial failure
- Works for all tables: snapshots, gpu, disk, network, errors
