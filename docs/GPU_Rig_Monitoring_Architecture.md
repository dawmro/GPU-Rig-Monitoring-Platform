...
| GPU (×N) | 16 JSON arrays (model/temp/util/fan/clocks/mem/power/PCIe) |
...
- `htmx_metrics` (Live Metrics): Reads `LatestSnapshot` + `LatestDockerContainer` + `DockerContainerMetric` + `GPUProcessMetric`. **0 timeseries queries.**
