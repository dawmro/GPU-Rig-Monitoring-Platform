...
class ReportDataView(APIView):
    ...
    def get(self, request, uuid):
        ...
        # -- GPU metrics: per-GPU aggregation --
        from django.db.models import Avg as _Avg, Max as _Max
        gpu_per_index = GPUMetric.objects.filter(**base_filter).values('gpu_index', 'model').annotate(
            gpu_temp_c_avg=_Avg('gpu_temp_c'),
            gpu_temp_c_max=_Max('gpu_temp_c'),
            gpu_util_pct_avg=_Avg('gpu_util_pct'),
            gpu_util_pct_max=_Max('gpu_util_pct'),
            power_draw_w_avg=_Avg('power_draw_w'),
            power_draw_w_max=_Max('power_draw_w'),
            mem_used_mb_avg=_Avg('mem_used_mb'),
            mem_used_mb_max=_Max('mem_used_mb'),
            fan_speed_pct_avg=_Avg('fan_speed_pct'),
            fan_speed_pct_max=_Max('fan_speed_pct'),
            gpu_core_clock_mhz_avg=_Avg('gpu_core_clock_mhz'),
            gpu_core_clock_mhz_max=_Max('gpu_core_clock_mhz'),
            gpu_mem_clock_mhz_avg=_Avg('gpu_mem_clock_mhz'),
            gpu_mem_clock_mhz_max=_Max('gpu_mem_clock_mhz'),
        ).order_by('gpu_index')
        result['gpu_devices'] = list(gpu_per_index)
        ...
        snap_agg = snap_qs.aggregate(
            cpu_utilization_pct_avg=_Avg('cpu_utilization_pct'),
            cpu_utilization_pct_max=_Max('cpu_utilization_pct'),
            cpu_temp_c_avg=_Avg('cpu_temp_c'),
            cpu_temp_c_max=_Max('cpu_temp_c'),
            cpu_power_w_avg=_Avg('cpu_power_w'),
            cpu_power_w_max=_Max('cpu_power_w'),
            cpu_freq_current_mhz_avg=_Avg('cpu_freq_current_mhz'),
            cpu_freq_current_mhz_max=_Max('cpu_freq_current_mhz'),
            mem_used_bytes_avg=_Avg('mem_used_bytes'),
            mem_used_bytes_max=_Max('mem_used_bytes'),
            swap_used_bytes_avg=_Avg('swap_used_bytes'),
            swap_used_bytes_max=_Max('swap_used_bytes'),
        ...
        # -- Disk metrics: per-device breakdown --
        disk_per_device = list(
            StorageMetric.objects.filter(**base_filter)
            .values('device', 'mountpoint')
            .annotate(
                disk_usage_pct_max=_Max('usage_pct'),
                disk_read_bytes_sum=Sum('read_bytes_delta'),
                disk_write_bytes_sum=Sum('write_bytes_delta'),
                disk_read_iops_max=_Max('read_iops_delta'),
                disk_write_iops_max=_Max('write_iops_delta'),
                disk_utilization_pct_max=_Max('utilization_pct'),
            ).order_by('device')
        )
        result['disk_devices'] = disk_per_device

        # -- Network metrics: per-interface breakdown --
        net_per_iface = list(
        ...
        # -- Cost estimate based on user's electricity rate --
        try:
            from accounts.models import User as AccountUser
            user_obj = AccountUser.objects.get(pk=user.pk)
            rate = float(user_obj.electricity_rate_kwh)
            result['power_cost_estimate'] = round(result['power_total_kwh'] * rate, 2)
            result['electricity_rate_kwh'] = rate
        except Exception:
            result['power_cost_estimate'] = None
            result['electricity_rate_kwh'] = None
